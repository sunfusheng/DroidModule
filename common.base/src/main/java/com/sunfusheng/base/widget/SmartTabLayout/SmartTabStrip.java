...
/**
 * <p>
 * Forked from https://github.com/ogaclejapan/SmartTabLayout
 */
class SmartTabStrip extends LinearLayout {
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            int selectedStart = SmartTabUtil.getStart(selectedTab, indicatorWithoutPadding);
            int selectedEnd = SmartTabUtil.getEnd(selectedTab, indicatorWithoutPadding);
            int left;
            int right;
            if (isLayoutRtl) {
                left = selectedEnd;
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                int nextStart = SmartTabUtil.getStart(nextTab, indicatorWithoutPadding);
                int nextEnd = SmartTabUtil.getEnd(nextTab, indicatorWithoutPadding);
                if (isLayoutRtl) {
...
//                thickness = thickness * thicknessOffset;
    ...
    private void drawSeparator(Canvas canvas, int height, int tabCount) {
        ...
        for (int i = 0; i < tabCount - 1; i++) {
            ...
            int end = SmartTabUtil.getEnd(child);
            int endMargin = SmartTabUtil.getMarginEnd(child);
