package com.sunfusheng.base.widget.ScrollableLayout;
...
public class ScrollableLayout extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeadHeight = 0;
        if (mHeadView != null && mHeadView.isShown()) {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        int offsetX = (int) Math.abs(currentX - mDownX);
        int offsetY = (int) Math.abs(currentY - mDownY);
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (needCheckUpDown) {
                    if (offsetX > mTouchSlop && offsetX > offsetY) {
                        needCheckUpDown = false;
                        isUpDown = false;
                    } else if (offsetY > mTouchSlop && offsetY > offsetX) {
                        needCheckUpDown = false;
                        isUpDown = true;
                    }
                }

                if (isUpDown && offsetY > mTouchSlop && offsetY > offsetX && (!isSticky() || mHelper.isTop())) {
                ...
                mLastY = currentY;
                int moveDistanceX = (int) (ev.getRawX() - x_down);
                int moveDistanceY = (int) (ev.getRawY() - y_down);
            ...
            case MotionEvent.ACTION_UP:
                if (isUpDown && offsetY > mTouchSlop && offsetY > offsetX) {
                    mVelocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
                    float yVelocity = -mVelocityTracker.getYVelocity();
                    if (Math.abs(yVelocity) > mMinimumVelocity) {
                        mDirection = yVelocity > 0 ? Direction.UP : Direction.DOWN;
                        if (mDirection == Direction.UP && isSticky()) {
                        } else {
            ...
            case MotionEvent.ACTION_CANCEL:
                if (isUpDown && isClickHead && (offsetX > mTouchSlop || offsetY > mTouchSlop)) {
                    int action = ev.getAction();
                    ev.setAction(MotionEvent.ACTION_CANCEL);
                    boolean dispatchResult = super.dispatchTouchEvent(ev);
                    ev.setAction(action);
    ...
    public void scrollTo(int x, int y) {
        ...
        if (onScrollListener != null) {
            onScrollListener.onScroll(deltaY, y, maxY);
        }
        super.scrollTo(x, y);
    }

    private void initOrResetVelocityTracker(MotionEvent ev) {
    ...
    private void initVelocityTrackerIfNotExists(MotionEvent ev) {
        if (mVelocityTracker == null) {
            mVelocityTracker = VelocityTracker.obtain();
        }
        mVelocityTracker.addMovement(ev);
    ...
    private int calcDuration(int duration, int timePassed) {
    ...
    public interface OnScrollListener {
        void onScroll(float scrollY, int offsetY, int maxY);
    }
