package com.sunfusheng.base.widget.SmartTabLayout;
...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            int start = (w - SmartTabUtil.getMeasuredWidth(firstTab)) / 2 - SmartTabUtil.getMarginStart(firstTab);
            int end = (w - SmartTabUtil.getMeasuredWidth(lastTab)) / 2 - SmartTabUtil.getMarginEnd(lastTab);
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        final boolean isLayoutRtl = SmartTabUtil.isLayoutRtl(this);
        View selectedTab = tabStrip.getChildAt(tabIndex);
        int widthPlusMargin = SmartTabUtil.getWidth(selectedTab) + SmartTabUtil.getMarginHorizontally(selectedTab);
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                int selectHalfWidth = SmartTabUtil.getWidth(selectedTab) / 2 + SmartTabUtil.getMarginEnd(selectedTab);
                int nextHalfWidth = SmartTabUtil.getWidth(nextTab) / 2 + SmartTabUtil.getMarginStart(nextTab);
            ...
            int x;
            if (isLayoutRtl) {
                int first = SmartTabUtil.getWidth(firstTab) + SmartTabUtil.getMarginEnd(firstTab);
                int selected = SmartTabUtil.getWidth(selectedTab) + SmartTabUtil.getMarginEnd(selectedTab);
                x = SmartTabUtil.getEnd(selectedTab) - SmartTabUtil.getMarginEnd(selectedTab) - extraOffset;
                x -= (first - selected) / 2;
            } else {
                int first = SmartTabUtil.getWidth(firstTab) + SmartTabUtil.getMarginStart(firstTab);
                int selected = SmartTabUtil.getWidth(selectedTab) + SmartTabUtil.getMarginStart(selectedTab);
                x = SmartTabUtil.getStart(selectedTab) - SmartTabUtil.getMarginStart(selectedTab) + extraOffset;
                x -= (first - selected) / 2;
            }
        ...
        if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
            ...
            if (isLayoutRtl) {
                x = -SmartTabUtil.getWidthWithMargin(selectedTab) / 2 + getWidth() / 2;
                x -= SmartTabUtil.getPaddingStart(this);
            } else {
                x = SmartTabUtil.getWidthWithMargin(selectedTab) / 2 - getWidth() / 2;
                x += SmartTabUtil.getPaddingStart(this);
            }
        ...
        int start = SmartTabUtil.getStart(selectedTab);
        int startMargin = SmartTabUtil.getMarginStart(selectedTab);
        if (isLayoutRtl) {
            x += start + startMargin - extraOffset - getWidth() + SmartTabUtil.getPaddingHorizontally(this);
        } else {
     ...
     * {@link #setCustomTabView(SmartTabLayout.TabProvider)}
     */
