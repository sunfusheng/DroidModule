...
public class CircleProgressView extends ProgressBar {
     ...
     * 半径
     */
    private int mRadius = DisplayUtil.dip2px(20);
    /**
     * 起始角度
     */
     ...
     * 内部圆与外部圆间距
     */
    private int mInnerPadding = DisplayUtil.dip2px(1);
    /**
     * 外部圆环颜色
     */
     ...
     * 外层圆环宽度
     */
    private int mOuterSize = DisplayUtil.dip2px(1);
    ...
    private void obtainAttributes(AttributeSet attrs) {
        ...
        mProgressStyle = ta.getInt(R.styleable.CircleProgressView_cpv_progressStyle, PROGRESS_STYLE_NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize =
                (int) ta.getDimension(R.styleable.CircleProgressView_cpv_progressNormalSize, mNormalBarSize);
        mNormalBarColor =
                ta.getColor(R.styleable.CircleProgressView_cpv_progressNormalColor, mNormalBarColor);

        mReachBarSize =
                (int) ta.getDimension(R.styleable.CircleProgressView_cpv_progressReachSize, mReachBarSize);
    ...
    public void setRadius(int radius) {
        mRadius = DisplayUtil.dip2px(radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = DisplayUtil.dip2px(innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = DisplayUtil.dip2px(outerSize);
        invalidate();
    }
